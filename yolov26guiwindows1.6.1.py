...
# Set modern global interface themes
ctk.set_appearance_mode("Dark")
ctk.set_default_color_theme("blue")


class QueueRedirector:
    """Thread-safe redirector that passes terminal output to a Queue and the original console."""
...
class YOLO26App(ctk.CTk):
    ...
    def create_header(self):
        ...
        gpu_text = f"⚡ GPU: {torch.cuda.get_device_name(0)}" if gpu_ok else "⚠️ CPU Mode Active"
    ...
    def save_recipe(self):
        ...
        try:
            with open(filename, 'w') as f: json.dump(recipe, f, indent=4)
    ...
    def load_recipe(self):
        ...
        if not filename: return
        try:
            with open(filename, 'r') as f: recipe = json.load(f)
    ...
    def setup_dataset_manager_tab(self):
        ...
        self.ds_report_text = ctk.CTkTextbox(report_frame, font=ctk.CTkFont(family="Consolas", size=12), wrap="word")
    ...
    def run_dataset_analysis(self, yaml_path):
        try:
            ...
            report.append("")
            report.append(f"{'✓' if stats['missing_lbls'] == 0 else '⚠'} Missing labels   {stats['missing_lbls']}")
            report.append(f"{'✓' if stats['corrupt_imgs'] == 0 else '⚠'} Corrupt images   {stats['corrupt_imgs']}")
            report.append(f"{'ℹ' if stats['zero_annots'] > 0 else '✓'} Zero annotations {stats['zero_annots']}")
            report.append(f"{'⚠' if imbalance_detected else '✓'} Class imbalance  {'Detected' if imbalance_detected else 'Normal'}")
            report.append("\nClass Distribution")
            report.append("────────────────────────────")
    ...
    def setup_training_tab(self):
        ...
        # Starting Weights selection with custom checkpoint browser
        ...
        # Batch Size Selection with Custom Batch Input
        ...
        self.console_text = ctk.CTkTextbox(console_frame, wrap="none", font=ctk.CTkFont(family="Consolas", size=11))
    ...
    def create_param_header(self, parent, label_text, tooltip_desc, rec_val="", row=None, col=None):
        """Creates a label paired with an info button displaying tooltips on hover and click."""
    ...
    def setup_advanced_hyperparams_tab(self):
        ...
        # Frame 1: Optimizer & Learning Rate
        ...
        # Frame 2: Training Mechanics & Layer Freezing Strategy
        ...
        # Training Freezing Strategy & Contextual Two-Stage Transfer Learning UI
        ...
        # 3. Freeze Backbone + Neck (Head Only / Linear Probing)
        ctk.CTkRadioButton(
            strategy_frame,
        ...
        # 4. Two-Stage Transfer Learning Option
        ctk.CTkRadioButton(
            strategy_frame,
        ...
        # Contextual Sub-Frame (nested directly below Two-Stage option)
        ...
        # Contextual Sub-Box (nested directly below Custom option)
        ...
        # Initialize initial strategy control states
        on_strategy_change()

        # Frame 3: Loss Weights
    ...
    def setup_augmentation_tab(self):
        ...
        # Card 1: Geometric Augmentation & Flips
        ...
        # Card 3: Advanced Composition Augmentations
    ...
    def get_gpu_telemetry(self):
        """Extracts live GPU utilization and VRAM usage."""
    ...
    def update_live_metrics_cards(self, df):
        """Updates the Live Metrics card widgets using the latest results.csv data."""
        try:
            if df.empty:
                return
    ...
    def run_training(self):
        ...
        try:
            ...
            aug_banner = (
                "\n" + "=" * 60 + "\n"
                "🛠️  YOLO26 ACTIVE DATA AUGMENTATION CONFIGURATION\n"
                + "-" * 60 + "\n"
                ...
                f"  • Rotation (degrees)       : ±{degrees_val:.1f}°\n"
            ...
            if selected_strategy == "Two-Stage Transfer Learning":
                ...
                # Stage 1: Frozen Backbone Alignment (Layers 0-9 locked)
                ...
                # Stage 2: Global Fine-Tuning (All Layers Unfrozen)
